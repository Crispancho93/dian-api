...
"""
Catálogos oficiales utilizados para la generación de documentos
electrónicos para la DIAN.

La aplicación debe utilizar los códigos como fuente de verdad.
Los nombres que se envían al XML se obtienen de estos catálogos.
"""


# ============================================================
# DEPARTAMENTOS DE COLOMBIA
# Código DIVIPOLA / DIAN -> Nombre del departamento
# ============================================================
...
#
# Código DIVIPOLA completo -> Nombre del municipio
#
# Ejemplo:
#
# 76001 -> Cali
...
#
# IMPORTANTE:
# El código completo de municipio está compuesto por:
#
...
def get_department_name(code: str) -> str:
    """
    Obtiene el nombre oficial del departamento a partir
    de su código.

    Ejemplo:

        get_department_name("76")
    ...
    if code not in DEPARTAMENTOS_DIAN:
        raise ValueError(
            f"Código de departamento no configurado: {code}"
        )

    return DEPARTAMENTOS_DIAN[code]

def get_municipality_name(code: str) -> str:
    """
    Obtiene el nombre oficial del municipio a partir
    de su código DIVIPOLA completo.

    Ejemplo:

        get_municipality_name("76001")
    ...
    if code is None:
        raise ValueError(
            "El código del municipio es obligatorio."
        )

    code = str(code).strip()

    if code not in MUNICIPIOS_DIAN:
        raise ValueError(
            f"Código de municipio no configurado: {code}"
        )

    return MUNICIPIOS_DIAN[code]