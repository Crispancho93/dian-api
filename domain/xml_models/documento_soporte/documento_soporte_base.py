...
class InvoiceBase:
    def __init__(self):
        self.root = etree.fromstring(templates_loader.template.xml_documentosoporte.encode('utf-8'))
    ...
    # def get_cuds(self, values):
    #    cuds_string = (
    #        values["NumDS"] + values["FecDS"] + values["HorDS"] + values["ValDS"] +
    ...
    #    )
    #    cuds_hash = hashlib.sha384(cuds_string.encode('utf-8')).hexdigest()
    #    return cuds_hash
    ...
    def add_invoice_line(self, line: dict):
        ...
        # parent = self.root.xpath('//ns:SupportDocument', namespaces={'ns': 'urn:DIAN:conisoft:facturaelectronica:DocumentoSoporte:DocumentoSoporte-2'})
        parent = [self.root]
        ...
        etree.SubElement(line_xml, f'{cbc}LineExtensionAmount', currencyID="COP").text = str(line["LineExtensionAmount"])
         ...
         # --- NUEVO: InvoicePeriod dentro de la línea (obligatorio para Documento Soporte) ---
        invoice_period = etree.SubElement(line_xml, f'{cac}InvoicePeriod')
        etree.SubElement(invoice_period, f'{cbc}StartDate').text = str(line.get("PeriodStartDate"))
        etree.SubElement(invoice_period, f'{cbc}DescriptionCode').text = str(line.get("PeriodDescriptionCode", "1"))
        etree.SubElement(invoice_period, f'{cbc}Description').text = str(line.get("PeriodDescription", "Por operación"))
        # -----------------------------------------------------------------------------------
        ...
        # --- NUEVO: Agrega el descuento por línea si es mayor a 0 ---
        ...
        # Es opcional y solo se informa cuando realmente existe.
